...
function showComments($pdo, $post_id, $parent_id = null, $margin = 0)
{
    ...
    $stmt = $pdo->prepare("SELECT c.*, u.username FROM comments c JOIN users u ON c.user_id = u.id WHERE c.post_id = ? AND c.parent_id " . ($parent_id === null ? "IS NULL" : "= ?") . " ORDER BY c.created_at ASC");
    $stmt->execute($parent_id === null ? [$post_id] : [$post_id, $parent_id]);
    $comments = $stmt->fetchAll(PDO::FETCH_ASSOC);

    foreach($comments as $comment) {
        echo "<div class='mb-3 p-2' style='margin-left:{$margin}px; border-left:2px solid #ccc;'></div>";
        ...
        // Forma za odgovor samo u slucaju da ulogovani korisnik nije autor komentara
